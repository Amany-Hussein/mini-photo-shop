...
Image Crop_Img(Image& image) {
    try {
        ...
        if (r < 0 || t < 0 || y <= 0 || h <= 0 || r + y > image.width || t + h > image.height) {
            throw runtime_error("Invalid cropping dimensions, please enter valid values.");
        }

        // Create a new image for the cropped portion
        Image croppedImage(y, h);

        // Copy pixels from the original image to the cropped image
        for (int u = 0; u < y; u++) {
            for (int l = 0; l < h; l++) {
                for (int p = 0; p < 3; p++) { // Assuming 3 channels (RGB)
                    croppedImage(u, l, p) = image(r + u, t + l, p);
                }
            }
        }

        // Optionally, you can save or display the cropped image here
        return croppedImage;
    } catch (const exception& e) {
        cerr << "Error: " << e.what() << endl;
        // Return an empty image in case of error
        //return Image(); // Assuming you have a constructor for an empty image
    }
}
